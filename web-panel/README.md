...
```text
@feierbuqiu/lora-mailbox-panel@0.2.1
```
...
Version `0.2.1` remains compatible with the original firmware heartbeat/status schema and also displays the calibrated retained fields published by the repository's optional `cloud-battery-monitor` Worker. No site-specific battery offset or MQTT credential is embedded in this package.
...
The current package tag is `lora-mailbox-panel-v0.2.1`.
